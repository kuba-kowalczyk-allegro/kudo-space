...
- Deliver login and logout with Supabase Auth using a single OAuth provider (GitHub) to satisfy `.ai/prd.md` FR-001.
...
- `src/layouts/Layout.astro`
  ...
  - Reserve `<slot name="auth-banner" />` for short-lived messages (e.g., "GitHub sign-in failed"), driven by query params or flash cookies.
...
- **First-time visitor**: redirected to `/login`, uses the configured provider (GitHub), completes Supabase OAuth flow, returns to original page.
...
- Provide descriptive but non-sensitive error copy (e.g., "Sign-in with GitHub failed. Try again." or provider-specific equivalent).

## 6. Testing Strategy (High-Level)

- Manual sign-in verification against the configured Supabase provider (GitHub by default).
...
- Document required environment variables:
  ...
  - `SUPABASE_OAUTH_PROVIDER="github"` to drive UI and validation.
- Supabase dashboard configuration:
  - Enable GitHub provider and set callback URL to `${SITE_URL}/auth/callback`.
